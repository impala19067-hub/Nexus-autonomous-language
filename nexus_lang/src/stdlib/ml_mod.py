...
class GPUModule:
    ...
    def _probe_devices(cls):
        """Detect available GPU/TPU devices. Falls back to CPU simulation."""
        if cls._devices:
            return
        ...
        try:
            ...
            pass
        # Simulate a virtual GPU device
        cls._devices.append(DeviceInfo(
            "cpu-sim", 0, "Sapphire Virtual GPU (CPU simulation)",
            8192.0, 8192.0
        ))
    ...
    def to_device(cls, tensor: Tensor, device: Union[str, int] = 0) -> Tensor:
        """Move a tensor to the specified device (simulated)."""
        cls._probe_devices()
