"""
🌌 SAPPHIRE LANGUAGE AUTONOMOUS TUTOR (Interactive CLI & Executable)
Learn Sapphire Language step-by-step through interactive lessons, quizzes, live REPL, and autonomous bot demos.
"""
...
# Include local sapphire_lang directory in path
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
SAPPHIRE_DIR = os.path.join(CURRENT_DIR, "sapphire_lang")
if os.path.exists(SAPPHIRE_DIR) and SAPPHIRE_DIR not in sys.path:
...
def execute_sapphire_snippet(code: str):
    """Utility to run inline Sapphire code and display output."""
    if not SAPPHIRE_ENGINE_AVAILABLE:
        print("⚠️ Sapphire interpreter engine unavailable.")
        return None
    try:
        lexer = Lexer(code)
        ...
        print(f"❌ Sapphire Runtime Error: {e}")
        return None

def chapter_1_basics():
    print_header("Chapter 1: Sapphire Syntax, Variables & Strings")
    print("""
Sapphire is a modern high-level language designed for PC Automation and Autonomous AI.

Key Concepts:
- Variables are declared with `let`: `let name = "Sapphire";`
...
"""
    print("\n[Executing Sapphire Code Snippet]:")
    print(code)
    print("-" * 40)
    execute_sapphire_snippet(code)
    print("-" * 40)

    # Quiz
    print("\n🧠 QUICK QUIZ:")
    print("How do you perform string interpolation in Sapphire?")
    print("1) 'Hello ' + name")
    print("2) 'Hello {name}'")
    print("3) 'Hello $name'")
    ans = input("Your choice (1-3): ").strip()
    if ans == "2":
        print("✅ Correct! Sapphire evaluates {expr} directly inside double-quoted strings.")
    else:
        print("❌ In Sapphire, use double quotes with braces: \"Hello {name}\".")

def chapter_2_control_flow():
    print_header("Chapter 2: Functions & Control Flow")
    print("""
Functions in Sapphire are defined using the `fn` keyword:
...
def chapter_3_stdlib():
    ...
    print("""
Sapphire provides first-class built-in modules for OS & PC control:
...
def chapter_4_parallel():
    ...
    print("""
Unlike languages with async/await syntax overhead, Sapphire supports native `parallel` blocks:
...
def chapter_5_ai():
    ...
    print("""
Sapphire has native AI primitives built into the runtime.
...
Sapphire automatically routes AI prompts to configured intelligence engines or embedded heuristic models.
""")
...
fn autonomous_health_check() {
    ...
    os.notify("Sapphire Autonomous Agent", status_summary);
...
def start_guided_course():
    print_header("Sapphire Interactive Guided Course")
    chapters = [
        chapter_1_basics,
...
def launch_repl():
    if not SAPPHIRE_ENGINE_AVAILABLE:
        ...
        return
    print_header("Interactive Sapphire REPL Engine")
    print("Type Sapphire expressions or code blocks. Type 'exit' to return to main menu.\n")
    interpreter = Interpreter()
...
def run_sample_bots():
    ...
    examples_dir = os.path.join(SAPPHIRE_DIR, "examples")
    if not os.path.exists(examples_dir):
        print("⚠️ Examples folder not found.")
        return
    files = [f for f in os.listdir(examples_dir) if f.endswith(".sp")]
    ...
    if choice.isdigit() and 1 <= int(choice) <= len(files):
        ...
        with open(target_file, 'r', encoding='utf-8') as f:
            content = f.read()
        execute_sapphire_snippet(content)
        print("-" * 50)

def generate_sample_files():
    print_header("Generate Sample Sapphire Scripts")
    output_path = os.path.join(CURRENT_DIR, "my_first_sapphire_bot.sp")
    sample_code = """// My First Autonomous AI Agent in Sapphire
fn main() {
    print("🤖 Initializing Autonomous Sapphire Agent...");
    ...
    os.notify("Sapphire Bot Status", "System state evaluated successfully!");
...
"""
    ...
    print("You can run this file anytime using: python sapphire_lang/src/cli.py run my_first_sapphire_bot.sp")

def main_menu():
    while True:
        print_header("Sapphire Autonomous AI & Language Interactive Tutor")
        print("1. 🎓 Guided Chapter Lessons & Quizzes")
        print("2. 🌌 Launch Interactive Sapphire REPL")
        print("3. 🤖 Run Pre-Built Sample Autonomous Bots")
        print("4. 💾 Generate Sample 'my_first_sapphire_bot.sp' Script")
        print("5. ℹ️  Sapphire Architecture & Language Overview")
        print("6. 🚪 Exit")
        ...
        if choice == "1":
            ...
            print_header("Sapphire Language Overview")
            print("""
Sapphire is an autonomous-first programming language created for seamless system control,
...
""")
            ...
            print("\nThank you for using Sapphire Language Autonomous Tutor! Happy Coding! 🚀\n")
            break
        else:
