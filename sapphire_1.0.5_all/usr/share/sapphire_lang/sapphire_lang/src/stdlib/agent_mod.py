...
class AgentBudget:
    ...
    @classmethod
    def from_dict(cls, value: Optional[dict]):
        if not value:
            return cls()
        budget = cls(**{key: value[key] for key in cls.__dataclass_fields__ if key in value})
        if min(budget.max_steps, budget.max_tokens, budget.max_time, budget.max_tool_calls) <= 0:
...
class AutonomousAgent:
    ...
    def __init__(self, name: str, goal: str, model_name: str = "default", context: Optional[str] = None,
                 budget: Optional[dict] = None, verifier: Optional[Callable] = None,
                 recovery: Optional[Callable] = None):
        self.name = name
        self.goal = goal
        self.model_name = model_name
        self.budget = AgentBudget.from_dict(budget)
    ...
    def run(self, max_steps: int = 10) -> dict:
        ...
        step_limit = min(max_steps, self.budget.max_steps)
        ...
        while not self.plan.is_finished() and step < step_limit:
            if time.monotonic() - started_at > self.budget.max_time:
                self.state = AgentState.FAILED
                self.log("⏱️ Agent budget reached: max_time")
                break
            step += 1
            ...
            try:
                ...
                if self.verifier is not None and not self.verifier(result, task, self):
                    raise RuntimeError(f"Verification failed for step {task.step_id}")
                ...
                if self.recovery is not None and self.recovery(error, task, self):
                    task.status = "pending"
        ...
        elif self.state not in (AgentState.FAILED, AgentState.RECOVERING):
            self.state = AgentState.FAILED
    ...
    def _execute_action(self, action: str, args: dict, description: str):
        if self.tool_calls >= self.budget.max_tool_calls:
            raise RuntimeError("Agent budget reached: max_tool_calls")
        self.tool_calls += 1
...
class AutonomyModule:
    ...
    @staticmethod
    def create_agent(name: str, goal: str, workspace: Optional[str] = None,
                    budget: Optional[dict] = None, verifier: Optional[Callable] = None,
                    recovery: Optional[Callable] = None) -> AutonomousAgent:
        return AutonomousAgent(name, goal, context=ContextEngine.build(goal, workspace=workspace),
                               budget=budget, verifier=verifier, recovery=recovery)
...
class AgentModule:
    ...
    def info() -> str:
        return (
            ...
            "  Pipeline     : Perception → Memory → Goals → Reasoning → Planning → Tools → Execution → Verification\n"
            "  Tool domains : OS, HTTP, Files, ML, notifications, and user-registered tools\n"
            "  Lifecycle    : OBSERVING → PLANNING → EXECUTING → VERIFYING → RECOVERING/REPLANNING\n"
            "  Controls     : budgets, permissions, structured decisions, memory, and recovery hooks\n"
