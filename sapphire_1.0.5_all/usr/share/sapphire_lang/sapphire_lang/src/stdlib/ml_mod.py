...
class Tensor:
    ...
    def __init__(self, data, dtype: str = "float32", requires_grad: bool = False):
        self._torch_data = data if torch is not None and isinstance(data, torch.Tensor) else None
        self._data = self._nested_to_float(data.tolist() if self._torch_data is not None else data)
        self.dtype = dtype
    ...
    def matmul(self, other: 'Tensor') -> 'Tensor':
        ...
        if self._torch_data is not None and other._torch_data is not None:
            return Tensor(torch.matmul(self._torch_data, other._torch_data), self.dtype)
    ...
    def tolist(self) -> list:
        if self._torch_data is not None:
            return self._torch_data.detach().cpu().tolist()
    ...
    def clone(self) -> 'Tensor':
        if self._torch_data is not None:
            return Tensor(self._torch_data.clone(), self.dtype, self.requires_grad)
        return Tensor(copy.deepcopy(self._data), self.dtype, self.requires_grad)

    @property
    def device(self) -> str:
        return str(self._torch_data.device) if self._torch_data is not None else "cpu"
...
class TorchTrainingResult:
    """Metrics and checkpoint state produced by the optional Torch backend."""

    def __init__(self, model, history: dict, device: str):
        self.model = model
        self.history = history
        self.device = device
        self.final_loss = history["train_loss"][-1] if history["train_loss"] else None
    ...
    def save(self, path: str) -> None:
        if torch is None:
            raise RuntimeError("Torch backend is not installed")
        torch.save({"model": self.model.state_dict(), "history": self.history, "device": self.device}, path)


class TorchTrainingModule:
    """Optional real autograd training backend powered by PyTorch."""
    ...
    @staticmethod
    def mlp(layer_sizes: list, activation: str = "relu"):
        if torch is None:
            raise RuntimeError("Torch backend requires PyTorch")
        activations = {"relu": torch.nn.ReLU, "sigmoid": torch.nn.Sigmoid, "tanh": torch.nn.Tanh}
        layers = []
        for index in range(len(layer_sizes) - 1):
            layers.append(torch.nn.Linear(layer_sizes[index], layer_sizes[index + 1]))
            if index < len(layer_sizes) - 2:
                layers.append(activations.get(activation.lower(), torch.nn.ReLU)())
        return torch.nn.Sequential(*layers)

    @staticmethod
    def fit(model, dataset: Dataset, epochs: int = 10, batch_size: int = 32,
            lr: float = 0.001, device: str = "auto", validation: Optional[Dataset] = None,
            checkpoint_path: Optional[str] = None, verbose: bool = True) -> TorchTrainingResult:
        if torch is None:
            raise RuntimeError("Torch backend requires PyTorch")
        ...
        for epoch in range(epochs):
            ...
            for batch in dataset.batch(batch_size):
                features = torch.tensor(batch["features"], dtype=torch.float32, device=selected)
                labels = torch.tensor(batch["labels"], dtype=torch.long, device=selected)
            ...
            if validation is not None:
                ...
                with torch.no_grad():
                    values = []
                    for batch in validation.batch(batch_size):
                        features = torch.tensor(batch["features"], dtype=torch.float32, device=selected)
                        labels = torch.tensor(batch["labels"], dtype=torch.long, device=selected)
                        values.append(float(loss_fn(model(features), labels).cpu()))
                    val_loss = sum(values) / max(1, len(values))
            history["train_loss"].append(train_loss)
            history["val_loss"].append(val_loss)
            if verbose:
                print(f"Torch epoch {epoch + 1}/{epochs}: loss={train_loss:.6f}")
        result = TorchTrainingResult(model, history, selected)
        if checkpoint_path:
...
class KernelModule:
    ...
    def gemm(A: Tensor, B: Tensor, alpha: float = 1.0, beta: float = 0.0,
        ...
        if np is not None:
            result = Tensor((alpha * (np.asarray(A.tolist()) @ np.asarray(B.tolist()))).tolist())
            if C is not None and beta != 0.0:
                result = Tensor((np.asarray(result.tolist()) + beta * np.asarray(C.tolist())).tolist())
            return result
        result = A.matmul(B)
        if alpha != 1.0:
    ...
    def fft(tensor: Tensor) -> Tensor:
        ...
        if N == 0:
            return Tensor([])
        if np is not None:
            return Tensor(np.abs(np.fft.fft(np.asarray(data, dtype=float))).tolist(), tensor.dtype)
        magnitudes = []
        for k in range(N):
...
class LocalComputeModule:
    """Real PC execution backend using CPU threads and optional NumPy kernels."""

    @staticmethod
    def info() -> dict:
        return {
            "backend": "numpy-openblas" if np is not None else "python-cpu",
            "workers": 1,
            "accelerated": np is not None,
        }

    @staticmethod
    def parallel_map(function: Callable, values: list, workers: int = 0) -> list:
        """Apply a callable across local CPU workers and preserve input order."""
        worker_count = workers or min(32, max(1, len(values)))
        if worker_count < 1:
    ...
    @staticmethod
    def chunked_map(function: Callable, values: list, workers: int = 0) -> list:
        """Run independent chunks locally, useful for CPU batch processing."""
        worker_count = workers or min(32, max(1, len(values)))
        chunks = [values[index::worker_count] for index in range(worker_count)]
        results = LocalComputeModule.parallel_map(lambda chunk: [function(value) for value in chunk], chunks, worker_count)
...
class GPUModule:
    ...
    def _probe_devices(cls):
        """Detect real CUDA devices and report CPU when unavailable."""
        if cls._devices:
            return
        if torch is not None and torch.cuda.is_available():
            ...
            return
        # Try to detect real CUDA devices via subprocess
        try:
            import subprocess
    ...
    def to_device(cls, tensor: Tensor, device: Union[str, int] = 0) -> Tensor:
        """Move a tensor to a real Torch device or CPU."""
        cls._probe_devices()
        dev_str = f"cuda:{device}" if isinstance(device, int) else device
        if dev_str.startswith("cuda"):
            if torch is None or not torch.cuda.is_available():
                raise RuntimeError("CUDA transfer requires a CUDA-enabled PyTorch installation")
            return Tensor(torch.tensor(tensor._data, dtype=torch.float32, device=dev_str), tensor.dtype, tensor.requires_grad)
        if dev_str != "cpu":
    ...
    def to_cpu(cls, tensor: Tensor) -> Tensor:
        """Move tensor back to CPU storage."""
        if tensor._torch_data is not None:
            return Tensor(tensor._torch_data.detach().cpu(), tensor.dtype, tensor.requires_grad)
        t_copy = tensor.clone()
        return t_copy
    ...
    def synchronize(cls, device: Union[str, int] = 0):
        """Synchronize a real CUDA device when available."""
        if torch is not None and torch.cuda.is_available():
    ...
    @classmethod
    def empty_cache(cls):
        """Release tracked allocations and the real CUDA allocator cache."""
...
class MLModule:
    ...
    train       = TrainModule
    torch_train = TorchTrainingModule
    kernel      = KernelModule
    cpu         = LocalComputeModule
    gpu         = GPUModule
