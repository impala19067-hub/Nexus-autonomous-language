...
# Resolve the directory that contains this file (i.e. sapphire_lang/src/)
_SRC_DIR = os.path.abspath(os.path.dirname(__file__))
# Resolve sapphire_lang/ (parent of src/)
_SAPPHIRE_LANG_DIR = os.path.abspath(os.path.join(_SRC_DIR, '..'))
# Resolve workspace root
_WORKSPACE_ROOT = os.path.abspath(os.path.join(_SAPPHIRE_LANG_DIR, '..'))
...
for _p in (_SAPPHIRE_LANG_DIR, _SRC_DIR, _WORKSPACE_ROOT):
...
VERSION = "1.0.5 (Local Runtime & ML Integrations)"
...
def main():
    ...
    if args.command == "run":
        ...
        print("Architecture: Language → Local Runtime → AI/ML Integrations → Bounded Agents")
        print("Capabilities: PC automation, concurrency, shell piping, AI service calls,")
        print("              tensors, autodiff, datasets, MLP training, and optional PyTorch/CUDA")
        print("              Distributed APIs provide planning/code generation; cluster execution is external.")
        print("Extension   : .sp")
