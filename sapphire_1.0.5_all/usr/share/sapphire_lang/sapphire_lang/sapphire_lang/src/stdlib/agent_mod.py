...
class PlanningModule:
    ...
    def react_step(goal: str, plan: Plan, observation: str) -> dict:
        """Ask the configured AI backend to choose the next action."""
        task = plan.next_task()
        if not task:
            return {"thought": "All tasks completed.", "action": "finish", "finished": True}
        from src.stdlib.ai_mod import AIModule
        prompt = (
            "Choose the next action for this plan. Return ONLY JSON with keys "
            "thought, action, and args. Action must be a registered tool name or "
            f"one of os, ml, notify. Goal: {goal}. Task: {task.description}. Observation: {observation}"
        )
        response = AIModule.prompt(prompt)
        decision = PlanningModule._parse_action(response)
        thought = decision.get("thought") or f"Observed '{observation}'. Executing: {task.description}."
        return {
            "thought": thought,
            "action": decision.get("action") or task.tool_name or "execute",
...
class AutonomousAgent:
    ...
    def run(self, max_steps: int = 10) -> dict:
        ...
        while not self.plan.is_finished() and step < max_steps:
            ...
            # 2. Execute the selected registered or built-in tool.
            task.status = "running"
            try:
                ...
                self.memory.push_chat("assistant", f"Executed {step_eval['action']}: {result}")
